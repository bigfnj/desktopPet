...
namespace DesktopPet
{
    ...
    /// <summary>
    /// Coordinates recoverable errors from independent domains. A success may clear only the
    /// domain generation it observed; the published value remains the newest still-active error.
    /// </summary>
    internal sealed class RecoverableErrorState<TDomain>
    {
        private sealed class DomainState
        {
            public long Generation;
            public long FailureOrder;
            public string Message = "";
        }

        private readonly object sync = new object();
        private readonly Dictionary<TDomain, DomainState> domains =
            new Dictionary<TDomain, DomainState>();
        private long failureOrder;

        public long CaptureGeneration(TDomain domain)
        {
            lock (sync)
                return GetDomain(domain).Generation;
        }
            ...
            Action<string> publish)
        {
            ...
            lock (sync)
            {
                DomainState state = GetDomain(domain);
                unchecked
                {
                    state.Generation++;
                    failureOrder++;
                }
                state.FailureOrder = failureOrder;
                state.Message = message ?? "";
                publish(CurrentMessageCore());
            }
        }
            ...
            Action<string> publish)
        {
            ...
            {
                DomainState state = GetDomain(domain);
                if (observedGeneration != state.Generation) return false;
                state.Message = "";
                state.FailureOrder = 0;
                publish(CurrentMessageCore());
                return true;
            }
        }
        ...
        private DomainState GetDomain(TDomain domain)
        {
            DomainState state;
            if (!domains.TryGetValue(domain, out state))
            {
                state = new DomainState();
                domains.Add(domain, state);
            }
            return state;
        }

        private string CurrentMessageCore()
        {
            long newestOrder = 0;
            string newestMessage = "";
            foreach (DomainState state in domains.Values)
            {
                if (state.Message.Length == 0 ||
                    state.FailureOrder < newestOrder)
                    continue;
                newestOrder = state.FailureOrder;
                newestMessage = state.Message;
            }
