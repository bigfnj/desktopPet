...
namespace DesktopPet
{
    ...
    internal static class SecuritySelfTest
    {
        public static bool Run(TextWriter output)
        {
            ...
            CheckFortunePackLoadLimits(ref failures, output);
            ...
            CheckFortuneGenerationOwnership(ref failures, output);
            CheckFortuneRandomAvailability(ref failures, output);
            CheckFortuneInitializationDisposal(ref failures, output);
            CheckSmartFortunePickDisposalBound(ref failures, output);
        ...
        private static void CheckFortunePackLoadLimits(
            ref int failures,
            TextWriter output)
        {
            List<TrustedPack> embedded;
            string error;
            Check(
                TrustedPackCatalog.TryLoad(out embedded, out error),
                "embedded trusted-pack catalog respects runtime load limits" +
                FormatError(error),
                ref failures,
                output);

            const string revision =
                "b52c11d184532364019ffc1756f3f0868ec99997";
            string pinnedUrl =
                "https://raw.githubusercontent.com/bigfnj/desktopPet/" +
                revision +
                "/packs/dadjokes.txt";
            var heldWithoutUrl = new TrustedPack
            {
                Id = "dadjokes",
                Url = null,
                RedistributionApproved = false
            };
            var heldWithUrl = new TrustedPack
            {
                Id = "dadjokes",
                Url = pinnedUrl,
                RedistributionApproved = false
            };
            var approvedWithoutUrl = new TrustedPack
            {
                Id = "dadjokes",
                Url = null,
                RedistributionApproved = true
            };
            var approvedWithUrl = new TrustedPack
            {
                Id = "dadjokes",
                Url = pinnedUrl,
                RedistributionApproved = true
            ...
            Check(
                TrustedPackCatalog.TryValidateRevision(
                    false,
                    null,
                    out revisionError) &&
                TrustedPackCatalog.TryValidateRevision(
                ...
                TrustedPackCatalog.TryValidateRevision(
                    ...
                    revision,
                    out revisionError) &&
                TrustedPackCatalog.TryValidateDistributionUrl(
                    heldWithoutUrl,
                    null,
                    out error) &&
                !TrustedPackCatalog.TryValidateDistributionUrl(
                ...
                TrustedPackCatalog.TryValidateDistributionUrl(
                    ...
                    revision,
                    out error),
                "held catalogs omit revision and URLs while approved packs require pinned values",
                ref failures,
                output);

            var exactPack = new List<TrustedPack>
            {
                ApprovedPack(
                    Ai.FortunePackLoadPolicy.MaximumFileBytes,
                    Ai.FortunePackLoadPolicy.MaximumEntries)
            };
            Check(
                TrustedPackCatalog.TryValidateLoadLimits(exactPack, out error),
                "trusted pack accepts exact per-file byte and row limits",
                ref failures,
                output);

            var overFileBytes = new List<TrustedPack>
            {
                ApprovedPack(
                    Ai.FortunePackLoadPolicy.MaximumFileBytes + 1,
                    1)
            };
            Check(
                !TrustedPackCatalog.TryValidateLoadLimits(
                    overFileBytes, out error),
                "trusted pack rejects per-file byte limit plus one",
                ref failures,
                output);

            var overFileEntries = new List<TrustedPack>
            {
                ApprovedPack(
                    1,
                    Ai.FortunePackLoadPolicy.MaximumEntries + 1)
            };
            Check(
                !TrustedPackCatalog.TryValidateLoadLimits(
                    overFileEntries, out error),
                "trusted pack rejects per-file row limit plus one",
                ref failures,
                output);

            var exactAggregate = new List<TrustedPack>();
            for (int index = 0; index < 4; index++)
                exactAggregate.Add(ApprovedPack(
                    Ai.FortunePackLoadPolicy.MaximumFileBytes,
                    Ai.FortunePackLoadPolicy.MaximumEntries / 4));
            Check(
                TrustedPackCatalog.TryValidateLoadLimits(
                    exactAggregate, out error),
                "trusted catalog accepts exact approved aggregate limits",
                ref failures,
                output);

            var overAggregateBytes = new List<TrustedPack>();
            for (int index = 0; index < 4; index++)
                overAggregateBytes.Add(ApprovedPack(
                    Ai.FortunePackLoadPolicy.MaximumFileBytes,
                    1));
            overAggregateBytes.Add(ApprovedPack(1, 1));
            Check(
                !TrustedPackCatalog.TryValidateLoadLimits(
                    overAggregateBytes, out error),
                "trusted catalog rejects approved aggregate bytes plus one",
                ref failures,
                output);

            var overAggregateEntries = new List<TrustedPack>
            {
                ApprovedPack(1, Ai.FortunePackLoadPolicy.MaximumEntries),
                ApprovedPack(1, 1)
            };
            Check(
                !TrustedPackCatalog.TryValidateLoadLimits(
                    overAggregateEntries, out error),
                "trusted catalog rejects approved aggregate rows plus one",
                ref failures,
                output);

            var exactFiles = ApprovedPacks(
                Ai.FortunePackLoadPolicy.MaximumFiles);
            Check(
                TrustedPackCatalog.TryValidateLoadLimits(
                    exactFiles, out error),
                "trusted catalog accepts exact file-count limit",
                ref failures,
                output);
            exactFiles.Add(ApprovedPack(1, 1));
            Check(
                !TrustedPackCatalog.TryValidateLoadLimits(
                    exactFiles, out error),
                "trusted catalog rejects file-count limit plus one",
                ref failures,
                output);
        }

        private static TrustedPack ApprovedPack(int bytes, int entries)
        {
            return new TrustedPack
            {
                Id = "boundary",
                Bytes = bytes,
                Count = entries,
        ...
        private static List<TrustedPack> ApprovedPacks(int count)
        {
            var packs = new List<TrustedPack>(count);
            for (int index = 0; index < count; index++)
                packs.Add(ApprovedPack(1, 1));
            return packs;
        }
        ...
        private static void CheckFortuneGenerationOwnership(
            ref int failures,
            TextWriter output)
        {
            var lifetime = new CancellationTokenSource();
            var blockedEntered = new ManualResetEventSlim(false);
            var blockedRelease = new ManualResetEventSlim(false);
            var initial = new DiagnosticOwnedValue();
            var stale = new DiagnosticOwnedValue();
            var current = new DiagnosticOwnedValue();
            int failureCount = 0;
            int staleObservedCancellation = 0;
            bool hiddenWhileReplacing = false;
            bool replacementPublished = false;
            bool staleCompleted = false;
            var coordinator =
                new GenerationOwnedValue<DiagnosticOwnedValue>(
                    lifetime.Token,
                    delegate(DiagnosticOwnedValue value) { value.Dispose(); },
            ...
            try
            {
                ...
                Task staleTask = coordinator.Start(
                    delegate(CancellationToken cancellationToken)
                    {
                        ...
                        if (cancellationToken.IsCancellationRequested)
                            Interlocked.Exchange(
                                ref staleObservedCancellation,
                                1);
                        return stale;
                    });
                blockedEntered.Wait(TimeSpan.FromSeconds(2));
                DiagnosticOwnedValue snapshot;
                hiddenWhileReplacing =
                    !coordinator.TryGetCurrent(out snapshot);

                Task replacementTask = coordinator.Start(
                    delegate(CancellationToken ignored)
                    {
                        return current;
                    });
                replacementTask.Wait(TimeSpan.FromSeconds(2));
                replacementPublished =
                    coordinator.TryGetCurrent(out snapshot) &&
                    ReferenceEquals(snapshot, current);

                blockedRelease.Set();
                staleCompleted = staleTask.Wait(TimeSpan.FromSeconds(2));
            }
            catch (Exception ex)
            {
                Check(
                    false,
                    "fortune generation ownership regression: " +
                        ex.GetType().Name + ": " + ex.Message,
                    ref failures,
                    output);
            }
            finally
            {
                blockedRelease.Set();
                coordinator.Dispose();
                blockedRelease.Dispose();
                blockedEntered.Dispose();
                lifetime.Dispose();
            }
            Check(
                hiddenWhileReplacing &&
                replacementPublished &&
                staleCompleted &&
                Volatile.Read(ref staleObservedCancellation) == 1 &&
                initial.DisposeCount == 1 &&
                stale.DisposeCount == 1 &&
                current.DisposeCount == 1 &&
                Volatile.Read(ref failureCount) == 0,
                "fortune generations cancel, publish atomically, and dispose stale candidates",
                ref failures,
                output);

            var timedLifetime = new CancellationTokenSource();
            var timedValue = new DiagnosticOwnedValue();
            TimeSpan timedReleaseBudget = TimeSpan.MinValue;
            var timedCoordinator =
                new GenerationOwnedValue<DiagnosticOwnedValue>(
                    timedLifetime.Token,
                    delegate(DiagnosticOwnedValue value) { value.Dispose(); },
                    null,
                    delegate(DiagnosticOwnedValue value, TimeSpan wait)
                    {
                        timedReleaseBudget = wait;
                        value.Dispose();
                    });
            try
            {
                timedCoordinator.Start(
                    delegate(CancellationToken ignored)
                    {
                        return timedValue;
                    }).Wait(TimeSpan.FromSeconds(2));
                timedCoordinator.Shutdown(TimeSpan.Zero);
            }
            finally
            {
                timedCoordinator.Dispose();
                timedLifetime.Dispose();
            }
            Check(
                timedReleaseBudget == TimeSpan.Zero &&
                timedValue.DisposeCount == 1 &&
                StartUp.RemainingShutdownBudget(
                    TimeSpan.FromSeconds(3),
                    TimeSpan.FromSeconds(2)) == TimeSpan.FromSeconds(1) &&
                StartUp.RemainingShutdownBudget(
                    TimeSpan.FromSeconds(3),
                    TimeSpan.FromSeconds(4)) == TimeSpan.Zero,
                "shutdown ownership propagates one aggregate remaining-time budget",
                ref failures,
                output);

            string directory = Path.Combine(
                Path.GetTempPath(),
                "DesktopPet-cache-cancel-" + Guid.NewGuid().ToString("N"));
            bool cacheCancellationObserved = false;
            bool cacheCancellationBounded = false;
            IDisposable held = null;
            var stopwatch = Stopwatch.StartNew();
            try
            {
                Directory.CreateDirectory(directory);
                string cachePath = Path.Combine(directory, "cache.bin");
                string mutexName = CrossSessionLock.BuildGlobalMutexName(
                    "VectorCache",
                    cachePath);
                held = CrossSessionLock.TryAcquire(
                    mutexName,
                    cachePath,
                    1000);
                if (held != null)
                {
                        ...
                        new CancellationTokenSource())
                    {
                        ...
                        {
                            cacheCancellationObserved = true;
                        }
                    }
                }
            }
            catch (Exception ex)
            {
                Check(
                    false,
                    "vector-cache cancellation regression: " +
                        ex.GetType().Name + ": " + ex.Message,
                    ref failures,
                    output);
            }
            ...
            Check(
                held != null &&
                cacheCancellationObserved &&
                cacheCancellationBounded,
                "vector-cache lock acquisition observes generation cancellation",
                ref failures,
                output);
        }

        private static void CheckFortuneRandomAvailability(
            ref int failures,
            TextWriter output)
        {
            var initializationEntered = new ManualResetEventSlim(false);
            var initializationRelease = new ManualResetEventSlim(false);
            var cancellation = new CancellationTokenSource();
            StartUp.FortuneRuntimeState state = null;
            Task initialization = null;
            int reportedFailures = 0;
            bool entered = false;
            bool randomAvailable = false;
            bool smartPending = false;
            bool completed = false;
            try
            {
                var provider = new Ai.FortuneProvider(
                    new[]
                    {
                        new Ai.FortuneEntry
                        {
                            ...
                            Genre = "quip",
                            Level = "general",
                            Text = "random is available",
                            Prof = false,
                            Custom = false
                        }
                    },
                    new Ai.AiSettings());
                state = new StartUp.FortuneRuntimeState(
                    provider,
                    delegate(Exception ignored)
                    {
                        Interlocked.Increment(ref reportedFailures);
                    });
                initialization = state.StartSmart(
                    cancellation.Token,
                    delegate(CancellationToken token)
                    {
                        ...
                        return (Ai.SmartFortunes)null;
                    });
                entered = initializationEntered.Wait(
                    TimeSpan.FromSeconds(2));
                randomAvailable =
                    state.Provider.Pick() == "random is available";
                smartPending =
                    state.Smart == null &&
                    initialization != null &&
                ...
                completed = initialization.Wait(
                    TimeSpan.FromSeconds(2));
            }
            catch (AggregateException ex)
            {
                if (ex.Flatten().InnerExceptions.Count != 1 ||
                    !(ex.Flatten().InnerExceptions[0] is
                        OperationCanceledException)) {
                    Check(
                        false,
                        "fortune random-availability regression: " +
            ...
            finally
            {
                ...
                initializationRelease.Dispose();
                initializationEntered.Dispose();
            }
            Check(
                entered &&
                randomAvailable &&
                smartPending &&
                completed &&
                Volatile.Read(ref reportedFailures) == 0,
                "random fortunes publish before optional smart initialization",
                ref failures,
                output);
        }

        private static void CheckFortuneInitializationDisposal(
            ref int failures,
            TextWriter output)
        {
            ...
            StartUp.FortuneRuntimeState state = null;
            Ai.SmartFortunes candidate = null;
            Task initialization = null;
            int reportedFailures = 0;
            bool bounded = false;
            bool pendingAtReturn = false;
            bool completed = false;
            try
            {
                var provider = new Ai.FortuneProvider(
                ...
                initialization = state.StartSmart(
                    ...
                    delegate(CancellationToken ignored)
                    {
                        entered.Set();
                        release.Wait();
                        candidate = new Ai.SmartFortunes(
                            CancellationToken.None);
                        return candidate;
                    });
                bool factoryEntered =
                ...
                Stopwatch stopwatch = Stopwatch.StartNew();
                state.Dispose(TimeSpan.FromMilliseconds(50));
                stopwatch.Stop();
                bounded =
                    factoryEntered &&
                ...
                Check(
                    ...
                    state.Smart == null &&
                    candidate != null &&
                    candidate.EmbedderDisposalCountForDiagnostics == 1 &&
                    Volatile.Read(ref reportedFailures) == 0,
                    "fortune state bounds initialization shutdown and eventually cleans the candidate",
                    ref failures,
                    output);
            }
            catch (Exception ex)
            {
                Check(
                    false,
                    "fortune initialization disposal threw " +
                        ex.GetType().Name + ": " + ex.Message,
                    ref failures,
                    output);
            }
            finally
            {
                ...
                if (initialization != null)
                    try { initialization.Wait(TimeSpan.FromSeconds(5)); }
                    catch { }
                if (state != null) state.Dispose();
        ...
        private static void CheckSmartFortunePickDisposalBound(
            ref int failures,
            TextWriter output)
        {
            var entered = new ManualResetEventSlim(false);
            var release = new ManualResetEventSlim(false);
            var smart = new Ai.SmartFortunes(CancellationToken.None);
            Task holder = null;
            try
            {
                holder = Task.Run(delegate
                {
                    smart.HoldEmbedLockForDiagnostics(
                        entered,
                        release);
                });
                bool lockHeld =
                    entered.Wait(TimeSpan.FromSeconds(2));
                Stopwatch stopwatch = Stopwatch.StartNew();
                smart.DisposeWithin(TimeSpan.FromMilliseconds(50));
                stopwatch.Stop();
                bool bounded =
                    lockHeld &&
                    stopwatch.Elapsed < TimeSpan.FromSeconds(1) &&
                    smart.EmbedderDisposalCountForDiagnostics == 0;

                release.Set();
                bool holderCompleted =
                    holder.Wait(TimeSpan.FromSeconds(2));
                bool disposalCompleted = SpinWait.SpinUntil(
                    delegate
                    {
                        return smart.EmbedderDisposalCountForDiagnostics == 1;
                    },
                    TimeSpan.FromSeconds(2));
                smart.Dispose();
                Check(
                    ...
                    holderCompleted &&
                    disposalCompleted &&
                    smart.EmbedderDisposalCountForDiagnostics == 1,
                    "smart-fortune disposal defers an active pick within budget exactly once",
                    ref failures,
                    output);
            }
            catch (Exception ex)
            {
                Check(
                    false,
                    "smart-fortune pick disposal threw " +
                        ex.GetType().Name + ": " + ex.Message,
                    ref failures,
                    output);
            }
            finally
            {
                release.Set();
                if (holder != null)
                    try { holder.Wait(TimeSpan.FromSeconds(2)); }
                    catch { }
                smart.Dispose();
                release.Dispose();
                entered.Dispose();
