...
{
    // CrossSessionLock + AtomicFile are copied verbatim from the base AppSettingsStore.cs (S3) so the
    // relocated engine (SmartFortunes/VectorCache + FortuneFileImporter) has no dependency on the base
    // assembly. Both are self-contained (framework types only). Kept in namespace DesktopPet so the
    // relocated DesktopPet.Ai engine resolves them by simple name, exactly as it did in the base.
